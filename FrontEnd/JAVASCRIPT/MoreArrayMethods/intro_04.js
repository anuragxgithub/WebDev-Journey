...
// Do sum of each items of array
const arr = [1,2,3,4]
let oneTime = true;
const sumResult = arr.reduce((res, val) => {
    if(oneTime) {
        console.log(res)  // result point to first elemnt
        console.log(val)  // val point to second element
        oneTime = false;
    }
    return res+val; 
})
...
const max = nums.reduce((prev, curr) => {
    console.log(prev);
    console.log(curr);  // if single element then curr also point to first element ⭐
...
// Problem statement: print sum of all prices of each product
// M_1 using for of loop
let sum = 0
for(let obj of products){
    sum += obj.price;
}
console.log(sum)
// M_2 using reduce() method
const total_price = products.reduce((accumulator, currentValue) => {  // currentValue or currentObj?
    return accumulator+currentValue.price;
},0)
console.log(total_price);

// Problem statement: print sum of all prices of each product which is in stock
const total_price_inStock = products.reduce((accumulator, currentValue) => {  // currentValue or currentObj?
